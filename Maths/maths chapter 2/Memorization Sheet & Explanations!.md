...
## 🧠 Your Ultimate Memorization Sheet & Explanations! (With Examples)
...
*   **Explanation:** These functions "undo" each other, but only if the input angle `x` (for $\sin^{-1}(\sin x)$) or value `x` (for $\sin(\sin^{-1} x)$) is within the specified valid range/domain. **The conditions are vital!**
    ...
    1.  $\sin^{-1} (\sin x) = x$, **if $x \in [-\frac{\pi}{2}, \frac{\pi}{2}]$**
        *   **Example:** $\sin^{-1}\left(\sin \frac{\pi}{4}\right) = \frac{\pi}{4}$ (since $\frac{\pi}{4} \in [-\frac{\pi}{2}, \frac{\pi}{2}]$).
        *   **Counter-example:** $\sin^{-1}\left(\sin \frac{3\pi}{4}\right) \neq \frac{3\pi}{4}$ because $\frac{3\pi}{4}$ is NOT in $[-\frac{\pi}{2}, \frac{\pi}{2}]$. Instead, $\sin \frac{3\pi}{4} = \sin (\pi - \frac{\pi}{4}) = \sin \frac{\pi}{4}$. So, $\sin^{-1}\left(\sin \frac{3\pi}{4}\right) = \sin^{-1}\left(\sin \frac{\pi}{4}\right) = \frac{\pi}{4}$.
    2.  $\cos^{-1} (\cos x) = x$, **if $x \in [0, \pi]$**
        *   **Example:** $\cos^{-1}\left(\cos \frac{2\pi}{3}\right) = \frac{2\pi}{3}$ (since $\frac{2\pi}{3} \in [0, \pi]$).
    3.  $\tan^{-1} (\tan x) = x$, **if $x \in (-\frac{\pi}{2}, \frac{\pi}{2})$**
        *   **Example:** $\tan^{-1}\left(\tan (-\frac{\pi}{6})\right) = -\frac{\pi}{6}$ (since $-\frac{\pi}{6} \in (-\frac{\pi}{2}, \frac{\pi}{2})$).
        *(Similarly for $\csc^{-1}, \sec^{-1}, \cot^{-1}$ with their respective ranges)*

    4.  $\sin (\sin^{-1} x) = x$, **if $x \in [-1, 1]$**
        *   **Example:** $\sin\left(\sin^{-1} \frac{1}{2}\right) = \frac{1}{2}$ (since $\frac{1}{2} \in [-1, 1]$).
    5.  $\cos (\cos^{-1} x) = x$, **if $x \in [-1, 1]$**
        *   **Example:** $\cos\left(\cos^{-1} (-0.8)\right) = -0.8$ (since $-0.8 \in [-1, 1]$).
    6.  $\tan (\tan^{-1} x) = x$, **if $x \in \mathbb{R}$**
        *   **Example:** $\tan\left(\tan^{-1} 5\right) = 5$ (since $5 \in \mathbb{R}$).
...
*   **Explanation:** These are some of the most frequently used formulas for simplifying and solving equations. The $2\tan^{-1}x$ formulas are particularly versatile.
    ...
    5.  $2\tan^{-1} x = \cos^{-1} \left(\frac{1-x^2}{1+x^2}\right)$, if $x \ge 0$
        *   **Example:** $2\tan^{-1}(1) = \cos^{-1}\left(\frac{1-1^2}{1+1^2}\right) = \cos^{-1}\left(\frac{0}{2}\right) = \cos^{-1}(0) = \frac{\pi}{2}$. (Matches the previous example).
